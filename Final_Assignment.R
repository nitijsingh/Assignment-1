...
d5$X<-NULL
# Defining dependent and indenpendt variable
Y4 <- cbind(d5$GDP.Growth)
X4 <- cbind(d5$ContriICT, d5$ContriNICT)
# Descriptive statistics
summary(Y4)
summary(X4)
# Set data as pannel data
...
summary(pooling)
# Putting control variable for Export % of GDP
Y5 <- cbind(d5$GDP.Growth)
X5 <- cbind(d5$ContriICT, d5$ContriNICT, d5$Export.of.goods.and.services....of.GDP.)
# Pooled OLS Esitmater with control variable for developed countries 
